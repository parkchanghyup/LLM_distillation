...
from utils.config_utils import (
    ROOT_DIR, MODEL_A_MERGED_DIR, MODEL_B_MERGED_DIR,
...
def main():
    ...
    # 2. Model A 학습 (Q-LoRA)
    logger.info("Step 2: Training Model A using Q-LoRA...")
    model_a = train_model_a(str(llm_train_path), str(test_path))  # 테스트 데이터를 평가 데이터로 사용
    logger.info(f"Model A 학습 완료: {MODEL_A_MERGED_DIR}")

    # 3. vLLM을 사용하여 나머지 데이터에 대한 요약문 생성
    logger.info("Step 3: Loading Model A with vLLM and generating summaries for remaining data...")
    vllm_model = load_model_vllm(str(MODEL_A_MERGED_DIR))
    ...
    evaluation_results = evaluate_models(
        model_path=str(MODEL_B_MERGED_DIR),
